...
async def call_openai(code: str, user_message: str) -> str:
    ...
    # verify=False is used because of the self-signed certs generated by install.sh
...
async def run_all_reviews(code: str, user_message: str, models: List[str]) -> Dict[str, str]:
    tasks = []
    active_model_ids = []

    # TODO: Refactor this to dynamically match AVAILABLE_MODELS instead of hardcoding model checks.
    for model in models:
        if model == "gpt-4o-latest":
            tasks.append(call_openai(code, user_message))
            active_model_ids.append(model)
        elif model == "grok-3":
            tasks.append(call_groq(code, user_message))
            active_model_ids.append(model)
        elif model == "local/deepseek-r1:1.5b":
            tasks.append(call_ollama("deepseek-r1:1.5b", code, user_message))
            active_model_ids.append(model)
        elif model == "local/llama3.2":
            tasks.append(call_ollama("llama3.2", code, user_message))
            active_model_ids.append(model)
        else:
    ...
    reviews = {}
    for model_id, result in zip(active_model_ids, results):
...
async def code_reviewer(state: AgentState) -> dict:
    reviews = await run_all_reviews(state["code"], state.get("user_message", ""), state.get("models", []))
